...
def train(hyp):
    ...
    print("Image sizes %g train, %g test" % (imagsz, imagsz_test))
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        print(("\n" + "%10s" * 8) % ("Epoch", "gpu_mem", "GIoU", "obj", "cls", "total", "targets", "imag_size"))
        pbar = tqdm(enumerate(dataloader), total=nb)  # progress bar
        for i, (imags, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ni = i + nb * epoch  # number integrated batches (since train start)
            imags = imags.to(device).float() / 255.0  # uint8 to float32, 0 - 255 to 0.0 - 1.0
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ns = [math.ceil(x * sf / gs) * gs for x in imags.shape[2:]]  # new shape (stretched to gs-multiple)
                    imags = F.interpolate(imags, size=ns, mode="bilinear", align_corners=False)
            ...
            s = ("%10s" * 2 + "%10.4g" * 6) % (
                "%g/%g" % (epoch, epochs - 1), mem, *mloss, targets.shape[0], imags.shape[-1])
            pbar.set_description(s)

            # Plot
            if ni < 3:
                f = "train_batch%g.jpg" % i  # filename
                res = plot_images(images=imags, targets=targets, paths=paths, fname=f)
                if tb_writer:
                    tb_writer.add_image(f, res, dataformats="HWC", global_step=epoch)
                    # tb_writer.add_graph(model, imags)  # add model to tensorboard
    ...
    if len(n):
        ...
        for f1, f2 in zip([wdir + "last.pth", wdir + "best.pth", "results.txt"], [flast, fbest, fresults]):
