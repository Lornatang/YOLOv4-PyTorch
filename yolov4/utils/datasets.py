...
help_url = "https://github.com/Lornatang/YOLOv4-PyTorch/Train-Custom-Data"
...
class LoadImages:  # for inference
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            assert image0 is not None, "Image Not Found " + path
            print("image %g/%g %s: " % (self.count, self.nF, path), end="")

        # Padded resize
        image = letterbox(image0, new_shape=self.image_size)[0]

        # Convert
        image = image[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
        image = np.ascontiguousarray(image)

        # cv2.imwrite(path + ".letterbox.jpg", 255 * image.transpose((1, 2, 0))[:, :, ::-1])  # save letterbox image
        return path, image, image0, self.cap
...
class LoadWebcam:  # for inference
    ...
    def __next__(self):
        ...
        if self.pipe == 0:  # local camera
            ret_val, image0 = self.cap.read()
            image0 = cv2.flip(image0, 1)  # flip left-right
        else:  # IP camera
            n = 0
            while True:
                n += 1
                ...
                if n % 30 == 0:  # skip frames
                    ret_val, image0 = self.cap.retrieve()
                    if ret_val:
                        break
        ...
        return image_path, image, image0, None
...
class LoadStreams:  # multiple IP or RTSP cameras
    ...
    def __next__(self):
        self.count += 1
        image0 = self.images.copy()
        if cv2.waitKey(1) == ord("q"):  # q to quit
            cv2.destroyAllWindows()
            raise StopIteration

        # Letterbox
        image = [letterbox(x, new_shape=self.image_size, auto=self.rect)[0] for x in image0]
        ...
        return self.sources, image, image0, None
...
class LoadImagesAndLabels(Dataset):  # for training/testing
        ...
        for i, file in enumerate(pbar):
            if labels_loaded:
                ...
                try:
                    ...
                    nm += 1  # print("missing labels for image %s" % self.image_files[i])  # file missing
                    continue
            ...
            if l.shape[0]:
                ...
                if create_datasubset and ns < 1E4:
                    ...
                    if exclude_classes not in l[:, 0]:
                        ns += 1
                        # shutil.copy(src=self.image_files[i], dst="./datasubset/images/")  # copy image
                ...
                ne += 1  # print("empty labels for image %s" % self.image_files[i])  # file empty
                # os.system("rm "%s" "%s"" % (self.image_files[i], self.label_files[i]))  # remove
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(image), labels_out, self.image_files[index], shapes
...
def reduce_image_size(path="../data/sm4/images", image_size=1024):  # from utils.datasets import *; reduce_image_size()
    # creates a new ./images_reduced folder with reduced size images of maximum size image_size
    ...
    for f in tqdm(glob.glob("%s/*.*" % path)):
        try:
            image = cv2.imread(f)
            h, w = image.shape[:2]
            r = image_size / max(h, w)  # size ratio
            if r < 1.0:
                image = cv2.resize(image, (int(w * r), int(h * r)), interpolation=cv2.INTER_AREA)  # _LINEAR fastest
            fnew = f.replace(path, path_new)  # .replace(Path(f).suffix, ".jpg")
            cv2.imwrite(fnew, image)
        except:
...
def imagelist2folder(path="data/coco_64image.txt"):  # from utils.datasets import *; imagelist2folder()
